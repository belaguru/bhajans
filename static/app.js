...
class BelaGuruApp {
    ...
    setPage(page) {
        ...
        if (page === "home") {
            this.updateURL('home');
            this.applyFilters(); // Refresh filtered list when navigating home
            this.renderHome();
        } else if (page === "upload") {
            this._selectedTags = [];
            this.renderUpload();
...
${bhajan.lyrics.split('\n').map(line => line.trimStart()).join('\n')}
    ...
    getDailyBhajan() {
        ...
        // Filter bhajans that have this day tag (match "Monday" or "Monday Bhajans")
        const dayBhajans = this.bhajans.filter(b => 
            b.tags && Array.isArray(b.tags) && b.tags.some(tag => 
                typeof tag === 'string' && tag.includes(dayName)
            )
        );
